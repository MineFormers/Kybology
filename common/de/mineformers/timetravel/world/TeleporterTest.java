...
public class TeleporterTest extends Teleporter {
	...
	public void placeInPortal(Entity par1Entity, double par2, double par4,
		...
		if (par1Entity instanceof EntityPlayer) {
			...
			if (TravellingRegistry
				...
				if (player.getHeldItem() != null
				        && player.getHeldItem().itemID == 5230) {
					ItemStack stopWatch = player.getHeldItem();
					NBTTagCompound nbt = stopWatch.getTagCompound();
					if (nbt == null) {
						nbt = new NBTTagCompound();
						stopWatch.setTagCompound(nbt);
					} else {
						int[] coords = nbt.getIntArray("Coordinates");
						boolean inDim = coords[3] == 1;
						if (inDim) {
							x = coords[0] + 0.5D;
							y = coords[1] + 1;
							z = coords[2] + 0.5D;
							...
							coords[3] = 0;
							nbt.setIntArray("Coordinates", coords);
		...
		par1Entity.setLocationAndAngles(x, y + 1, z, par1Entity.rotationYaw, 0.0F);
